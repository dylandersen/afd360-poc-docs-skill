...
2. **Offer optional sections** — Before scaffolding, run `node scripts/scaffold.mjs --list-sections` to discover what's available, then ask the SE which (if any) they want included now. Phrase it like:

   > "Want any of these sections seeded now? You can also add them later with `/update-docs`. Skip anything you don't have answers for yet."

   Present the list as a multi-select. Common picks for handoff: `security`, `faq`, `glossary`, `release-notes`. Common picks for stakeholder demos: `demo-script`. The full list is whatever the script prints — do not invent slugs.

   **It is fine for the SE to skip all of them.** Default to skip if they're unsure or short on answers — sections are easy to add later.

3. **Scaffold** — Call `scripts/scaffold.mjs` from the skill directory with the collected values. If the SE picked any optional sections, pass them as `--include-sections "slug1,slug2,..."`. Do not reimplement the copy/replace logic.
...
5. **Next steps** — Print a handoff checklist: where to edit content, how to add pages, how to swap the logo, how to deploy, and a reminder that `/update-docs add-section <slug>` works for any section the SE skipped now.
